...
class LivePriceFetcher:
    """Fetches LIVE XAU/USD price from multiple free API sources"""
    ...
    def fetch_goldprice_org(self):
        """Source 1: GoldPrice.org data feed (real-time)"""
        try:
            ...
            if resp.status_code == 200:
                ...
                if 'items' in data and len(data['items']) > 0:
                    ...
                    if gold_price and gold_price > 1000:
                        return float(gold_price), "GoldPrice.org"
        except Exception:
            pass
        return None, None

    def fetch_metals_live(self):
        """Source 2: Metals.live API (real-time spot)"""
        try:
    ...
    def fetch_frankfurter(self):
        """Source 3: Frankfurter API (daily, fallback)"""
        try:
    ...
    def get_live_price(self):
        """Try multiple free API sources for live gold price"""
        sources = [
            self.fetch_goldprice_org,
            self.fetch_metals_live,
            self.fetch_frankfurter,
        ]
        ...
        for source_func in sources:
            ...
            if price:
                ...
                # Build synthetic OHLC from live ticks
...
class TradingBot:
    ...
    def calculate_indicators(self, df):
        ...
        try:
            ...
            # Ensure ATR has a minimum value for gold
...
{CYAN}{BOLD}  ══════════════════════════════════════════════════{RESET}
  ...
  {YELLOW}💰 Pret LIVE:{RESET}          {WHITE}{BOLD}${price:.2f}{RESET}{price_change}
  {YELLOW}🕐 Ultima actualizare:{RESET} {DIM}{now}{RESET}
  {YELLOW}📡 Sursa date:{RESET}         {DIM}{self.fetcher.source}{RESET}
  {YELLOW}📊 Tick-uri colectate:{RESET} {DIM}{len(self.fetcher.prices_history)}{RESET}
...
{CYAN}{BOLD}  ═══════════════════════════════════════════���══════{RESET}
  ...
  {YELLOW}📡 Se colecteaza date LIVE pentru indicatori...{RESET}

  {WHITE}💰 Pret curent:{RESET}  {BOLD}{price_str}{RESET}
  {WHITE}📡 Sursa:{RESET}        {DIM}{self.fetcher.source}{RESET}
  {WHITE}📊 Progres:{RESET}      [{bar}] {pct}%
  {WHITE}📈 Tick-uri:{RESET}     {count}/{needed}
...
"""
        ...
        MIN_TICKS = 35  # Minimum data points needed for indicators
        ...
        while True:
            try:
                ...
                if price is None:
                    os.system('cls' if os.name == 'nt' else 'clear')
                    print(f"\n  {RED}⚠️  Nu s-a putut obtine pretul LIVE.{RESET}")
                    print(f"  {YELLOW}Verificati conexiunea la internet.{RESET}")
                    print(f"  {DIM}Se reincearca in 5 secunde...{RESET}")
                    time.sleep(5)
                    continue

                self.tick_count = len(self.fetcher.prices_history)

                # Warmup phase - collecting enough data for indicators
                if self.tick_count < MIN_TICKS:
                    self.display_warmup(self.tick_count, MIN_TICKS)
                    time.sleep(10)
                    continue

                # Get DataFrame and calculate indicators
